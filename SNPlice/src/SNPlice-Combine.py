#!/bin/env python27
import sys, os, os.path, glob, copy, traceback, time, re, csv, tempfile, urllib, shutil, atexit, subprocess, time
from collections import defaultdict
sys.path.append(os.path.join(os.path.split(os.path.abspath(__file__))[0],'..','..','common','src'))
...
if opt.output:
    ...
    if extn == 'csv':
        ...
        output = XLSFileTable(filename=filename,headers=outheaders,sheet='Results')
    elif extn == 'xlsx':
        output = XLSXFileTable(filename=filename,headers=outheaders,sheet='Results')         
    elif extn == 'txt':
...
for (snpstr,junc),r in sorted(countdata.iteritems()):
        ...
        p = emptysym; pval = emptysym; lodval = emptysym
        if junc and \
	   (nsnpi+nsnpex) > 0 and \
           (nwti+nwtex) > 0 and \
           (nsnpi+nwti) > 0:
	    ...
	    l = lod(nsnpi, (nsnpi+nsnpex), (nsnpi+nwti), (nsnpi+nsnpex+nwti+nwtex))
	    if l != None:
		lodval = l
