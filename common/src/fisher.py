...
def lod(x,N,n,M,pseudocount=0.5,base=2.0):
    n00 = x; n01 = (N-x); n10=(n-x); n11=(M-n00-n01-n10);
    if min(n00,n01,n10,n11) < 0 or \
	(pseudocount == 0.0 and min(n00,n01,n10,n11) <= 0):
        return None
    n00 += pseudocount; n01 += pseudocount;
    n10 += pseudocount; n11 += pseudocount;
...
if __name__ == '__main__':
	...
	sl = map(int,l.split())
	    ...
	    continue
        x = sl[0]; N = sl[0] + sl[1];
        n = sl[0] + sl[2]; M = sum(sl)
	v = (lod(x,N,n,M), fisher_exact_low(x,N,n,M), fisher_exact_high(x,N,n,M), fisher_exact(x,N,n,M))
