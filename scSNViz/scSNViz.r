...
# Check if copykat_cairo.r exists if copykat option is enabled
if (args$`enable-copykat` && !file.exists('copykat_cairo.r')) {
  error.msg <- paste(error.msg, '- copykat_cairo.r file is required when enabling CopyKat (-u).', sep='\n')
}
...
if (args$`enable-copykat`) {
  suppressPackageStartupMessages({
    library('copykat')
    source('copykat_cairo.r')
  })

    print('Running CopyKat. This may take a while...')
    cts <- GetAssayData(srt, layer='counts', assay='SCT')
    ckt <- copykat_cairo(cts, sam.name=sample.name)
...
# directories (if they doesn't exist) for saving each plots as seperate html 
...
if (num.snvs > 1) {
  hist_list <- list(
    list(aes = aes(x = TotalVAF), xlab = 'TotalVAF', file_suffix = 'TotalVAF'),
    list(aes = aes(x = MeanVAF), xlab = 'MeanSNVsVAF', file_suffix = 'MeanSNVsVAF'),
    list(aes = aes(x = SNVCount), xlab = 'N_VARreadCounts', file_suffix = 'N_VARreadCounts'),
    list(aes = aes(x = SNV.N), xlab = 'N_SNV', file_suffix = 'N_SNV')
  )
...
### ==================================================== DIMENSIONALITY REDUCTION scSNV PLOTS ========================================================== ###
...
generate_3d_plot <- function(data, color_column, color_scale, reversescale_option,
  ...
  # if (!is.null(title)) {
  #   plot <- plot %>% layout(title = title, titlefont = list(color = 'black')) 
  # }
...
if (args$`enable-sctype`) {
  ...
  # save the Cell types plot as an HTML file
...
generate_copykat_plot <- function(srt, dim_plotting, dim_title, reduction) {
  ...
  #   f <- f %>% layout(title = title, titlefont = list(color = 'blue')) 
  # }
...
if (!args$`disable-ind-plots`) {
  ...
  #snv_options <- paste(snvs$CHROM, snvs$POS, paste(snvs$REF, snvs$ALT, sep=">"), sep = ":")

  # function to generate plots for a selected SNV
  generate_plots <- function(selected_snv) {
  ...
  save_snv_plot <- function(plot, snv, plot_type) {
    ...
    indsnv_with_title <- plot %>% layout(title = list(text = indsnv_title, font = list(color = 'black')), margin = list(t = 50))
