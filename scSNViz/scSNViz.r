...
f <- f %>% add_markers(data=df.3dplot[df.3dplot[['Undetected']]==0,], 
                                      x=~eval(parse(text = paste0(dim.plotting,'_1'))), y=~eval(parse(text = paste0(dim.plotting,'_2'))), z=~eval(parse(text = paste0(dim.plotting,'_3'))),
...
if (!args$`disable-slingshot`) {
  f <- f %>% add_trace(data=curves, x=~eval(parse(text = paste0(dim.plotting,'_1'))), y=~eval(parse(text = paste0(dim.plotting,'_2'))), z=~eval(parse(text = paste0(dim.plotting,'_3'))),
...
saveWidget(f, paste0(dir.name, sample.name, '_TotalVAF',
                     param.name, '.html'),
           selfcontained=F, libdir='lib')
}
...
f <- f %>% add_markers(data=df.3dplot[df.3dplot[['Undetected']]==0,], 
                       ...
                       marker=list(color=~MedianVAF, colorscale=color.scale3,
                                   ...
                                   line=list(color=~MedianVAF,
                                             width=cell.size)))
f <- f %>% add_markers(data=df.3dplot[df.3dplot[['Undetected']]==1,],
                       x=~eval(parse(text = paste0(dim.plotting,'_1'))), y=~eval(parse(text = paste0(dim.plotting,'_2'))), z=~eval(parse(text = paste0(dim.plotting,'_3'))), size=0.05,
...
saveWidget(f, paste0(dir.name, sample.name, '_MedianVAF',
                     param.name, '.html'),
           selfcontained=F, libdir='lib')
}
...
saveWidget(f, paste0(dir.name, sample.name, '_MeanVAF',
                     param.name, '.html'),
           selfcontained=F, libdir='lib')
}
...
saveWidget(f, paste0(dir.name, sample.name, '_N_VARreadCounts',
                     param.name, '.html'),
           selfcontained=F, libdir='lib')
}
...
saveWidget(f, paste0(dir.name, sample.name, '_REFreadCounts',
                     param.name, '.html'),
           selfcontained=F, libdir='lib')
}
...
saveWidget(f, paste0(dir.name, sample.name, '_N_SNV',
                     param.name, '.html'),
           selfcontained=F, libdir='lib') 
}
...
f <- f %>% add_trace(data=df.3dplot,  x=~eval(parse(text = paste0(dim.plotting,'_1'))), y=~eval(parse(text = paste0(dim.plotting,'_2'))), z=~eval(parse(text = paste0(dim.plotting,'_3'))), size=0.05, opacity=0.50,
                     mode='markers',
                     color=~customclassif)
if (!args$`disable-slingshot`) {
f <- f %>% add_trace(data=curves,  x=~eval(parse(text = paste0(dim.plotting,'_1'))), y=~eval(parse(text = paste0(dim.plotting,'_2'))), z=~eval(parse(text = paste0(dim.plotting,'_3'))), mode='lines',
                     color=~factor(Lineage), line=list(width=4))
file.name <- paste0(sample.name, '-sctype_slingshot-lineages-all.html')
} else {
file.name <- paste0(sample.name, '-sctype.html')
}
if (args$`disable-3d-axis`) {
  f <- f %>% layout(scene=list(xaxis=list(title='', zeroline=F, showline=F,
                                          showticklabels=F, showgrid=F),
...
s <- slingshot(as.SingleCellExperiment(srt),
    clusterLabels='seurat_clusters', reducedDim=toupper(dimensionality.reduction))
# plot seurat clusters with individual slingshot trajectories
