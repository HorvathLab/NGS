...
3-dimensional UMAPs of SNV reads, reference reads, and VAF for each cell. The
...
param.name <- paste0('_umap_', th.vars, 'r', th.reads)
...
if (args$`enable-sctype`){
  ...
  df.umap <- as.data.frame(Embeddings(srt, reduction='umap'))
  df.3dplot <- merge(df.snv, df.umap, by=0)
  colnames(df.3dplot)[12:14] <- c('UMAP_1', 'UMAP_2', 'UMAP_3')
} else {
  df.snv <- as.data.frame(srt[[c('SNVCount', 'RefCount', 'TotalVAF',
  'MedianVAF', 'MeanVAF', 'SNV.N','seurat_clusters','HasSNV','Undetected')]])
  df.umap <- as.data.frame(Embeddings(srt, reduction='umap'))
  df.3dplot <- merge(df.snv, df.umap, by=0)
  colnames(df.3dplot)[11:13] <- c('UMAP_1', 'UMAP_2', 'UMAP_3')
}
...
colnames(curves)[1:3] <- c('UMAP_1', 'UMAP_2', 'UMAP_3')
if (num.snvs>1) {
  # generate histograms
...
if (!args$`disable-slingshot`) {
  f <- f %>% add_trace(data=curves, x=~UMAP_1, y=~UMAP_2, z=~UMAP_3,
...
f <- f %>% add_markers(data=df.3dplot[df.3dplot[['Undetected']]==0,], 
                       ...
                       marker=list(color=~MedianVAF, colorscale=color.scale3,
                                   ...
                                   line=list(color=~MedianVAF,
                                             width=cell.size)))
f <- f %>% add_markers(data=df.3dplot[df.3dplot[['Undetected']]==1,],
                       x=~UMAP_1, y=~UMAP_2, z=~UMAP_3, size=0.05,
...
f <- f %>% add_trace(data=df.3dplot, x=~UMAP_1, y=~UMAP_2, z=~UMAP_3, size=0.05, opacity=0.50,
                     mode='markers',
                     color=~customclassif)
if (!args$`disable-slingshot`) {
f <- f %>% add_trace(data=curves, x=~UMAP_1, y=~UMAP_2, z=~UMAP_3, mode='lines',
...
for (i in grep('slingPseudotime', colnames(s@colData))) {
  ...
  if (!args$`disable-slingshot`) {
  f <- f %>% add_trace(data=curve, x=~UMAP_1, y=~UMAP_2, z=~UMAP_3,
...
#make plot that contains VARReads, RefReads, UMAPs, Slingshot
...
for (i in 1:nrow(snvs)) {
  ...
  y <- data.frame(x=df.umap[, 1], y=df.umap[, 2], z=df.umap[, 3],
