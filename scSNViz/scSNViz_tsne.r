...
param.name <- paste0('_tsne_', th.vars, 'r', th.reads)
...
if (args$`enable-sctype`){
  ...
  df.tsne <- as.data.frame(Embeddings(srt, reduction='tsne'))
  df.3dplot <- merge(df.snv, df.tsne, by=0)
  colnames(df.3dplot)[12:14] <- c('tSNE_1', 'tSNE_2', 'tSNE_3')
} else {
  df.snv <- as.data.frame(srt[[c('SNVCount', 'RefCount', 'TotalVAF',
  'MedianVAF', 'MeanVAF', 'SNV.N','seurat_clusters','HasSNV','Undetected')]])
  df.tsne <- as.data.frame(Embeddings(srt, reduction='tsne'))
  df.3dplot <- merge(df.snv, df.tsne, by=0)
  colnames(df.3dplot)[11:13] <- c('tSNE_1', 'tSNE_2', 'tSNE_3')
}
...
sce <- slingshot(sce, clusterLabels='seurat_clusters', reducedDim='TSNE')

curves <- slingCurves(sce, as.df=T)
colnames(curves)[1:3] <- c('tSNE_1', 'tSNE_2', 'tSNE_3')
if (num.snvs>1) {
  # generate histograms
...
if (!args$`disable-slingshot`) {
  f <- f %>% add_trace(data=curves, x=~tSNE_1, y=~tSNE_2, z=~tSNE_3,
...
f <- f %>% add_markers(data=df.3dplot[df.3dplot[['Undetected']]==0,], 
                       ...
                       marker=list(color=~MedianVAF, colorscale=color.scale3,
                                   ...
                                   line=list(color=~MedianVAF,
                                             width=cell.size)))
f <- f %>% add_markers(data=df.3dplot[df.3dplot[['Undetected']]==1,],
                       x=~tSNE_1, y=~tSNE_2, z=~tSNE_3, size=0.05,
...
f <- f %>% add_trace(data=df.3dplot, x=~tSNE_1, y=~tSNE_2, z=~tSNE_3, size=0.05, opacity=0.50,
                     mode='markers',
                     color=~customclassif)
if (!args$`disable-slingshot`) {
f <- f %>% add_trace(data=curves, x=~tSNE_1, y=~tSNE_2, z=~tSNE_3, mode='lines',
...
s <- slingshot(as.SingleCellExperiment(srt),
    clusterLabels='seurat_clusters', reducedDim='TSNE')
...
for (i in grep('slingPseudotime', colnames(s@colData))) {
  ...
  if (!args$`disable-slingshot`) {
  f <- f %>% add_trace(data=curve, x=~tSNE_1, y=~tSNE_2, z=~tSNE_3,
...
#make plot that contains VARReads, RefReads, tSNEs, Slingshot
...
for (i in 1:nrow(snvs)) {
  ...
  y <- data.frame(x=df.tsne[, 1], y=df.tsne[, 2], z=df.tsne[, 3],
