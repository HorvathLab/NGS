...
f <- f %>% add_trace(data=df.3dplot, x=~PC_1, y=~PC_2, z=~PC_3, size=0.05, opacity=0.50,
                     mode='markers',
                     color=~customclassif)
if (!args$`disable-slingshot`) {
f <- f %>% add_trace(data=curves, x=~PC_1, y=~PC_2, z=~PC_3, mode='lines',
...
s <- slingshot(as.SingleCellExperiment(srt),
    clusterLabels='seurat_clusters', reducedDim='PCA')
...
for (i in grep('slingPseudotime', colnames(s@colData))) {
  ...
  if (!args$`disable-slingshot`) {
  f <- f %>% add_trace(data=curve, x=~PC_1, y=~PC_2, z=~PC_3,
