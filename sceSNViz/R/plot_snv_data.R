...
#' @importFrom copykat copykat CNA.MCMC annotateGenes.hg20 annotateGenes.mm10 baseline.GMM baseline.norm.cl baseline.synthetic convert.all.bins.hg20 heatmap.3
...
#' - **SNV Dimensionality Reduction Plot**: Makes a dimensionality reduction plot for SNVs instead of cells.
...
#' @export
#'
plot_snv_data <- function(seurat_object, plot_data, processed_snv, output_dir = NULL, include_histograms = TRUE,
                          include_cell_types = FALSE, include_snv_dim_red = TRUE, include_copykat = FALSE,
                          dimensionality_reduction = "UMAP", slingshot = TRUE, color_scale = "YlOrRd",
                          cell_size = 3, disable_3d_axis = FALSE, save_each_plot = FALSE) {
  ...
  reversescale_options <- c(TRUE, FALSE, TRUE, TRUE, FALSE, FALSE)
  ...
  for (metric in names(plot_titles)) {
    ...
    if (save_each_plot && !is.null(output_dir)) {
      saveWidget(as_widget(plot), file = file.path(
        output_dir, "Figures_Individual_Plots_HTML",
        paste0(file_title, ".html")), selfcontained = FALSE, libdir = "lib")
    }
  }
  ...
  if (include_cell_types && "customclassif" %in% colnames(plot_data)) {
    ...
    if (disable_3d_axis) {
      cell_type_plot <- cell_type_plot %>%
        layout(scene = list(xaxis = list(title = NULL, showticklabels = FALSE, zeroline = FALSE,
                                         showline = FALSE, showgrid = FALSE),
                            yaxis = list(title = NULL, showticklabels = FALSE, zeroline = FALSE,
                                         showline = FALSE, showgrid = FALSE),
                            zaxis = list(title = NULL, showticklabels = FALSE, zeroline = FALSE,
                                         showline = FALSE, showgrid = FALSE)))
    }
    else {
    ...
    if (save_each_plot) {
      ...
      saveWidget(as_widget(cell_type_plot), file = file.path(
        output_dir, "Figures_Individual_Plots_HTML", "Cell_types_scType.html"),
        ...
        if (!disable_3d_axis) {
          lineage_plot <- lineage_plot %>% layout(
            ...
            add_trace(data = curve,
                      ...
                      y = ~get(paste0(toupper(dimensionality_reduction), "_2")),
                      z = ~get(paste0(toupper(dimensionality_reduction), "_3")),
                      mode = "lines", showlegend = TRUE, line = list(width = 4, color = "black"))
        }
        else {
          lineage_plot <- lineage_plot %>% layout(
            scene = list(xaxis = list(title = NULL, showticklabels = FALSE, zeroline = FALSE,
                                      showline = FALSE, showgrid = FALSE),
                         yaxis = list(title = NULL, showticklabels = FALSE, zeroline = FALSE,
                                      showline = FALSE, showgrid = FALSE),
                         zaxis = list(title = NULL, showticklabels = FALSE, zeroline = FALSE,
  ...
  if (include_snv_dim_red) {
    ...
    if (save_each_plot && !is.null(output_dir)) {
      ...
      saveWidget(as_widget(snv_dim_red_plot), file = file.path(
        output_dir, "Figures_Individual_Plots_HTML", "SNV_Plot.html"),
  ...
  if (include_copykat) {
    ...
    if (save_each_plot) {
      ...
      saveWidget(as_widget(copykat_plot), file = file.path(
        output_dir, "Figures_Individual_Plots_HTML", "CopyKat_CNVs.html"),
        selfcontained = F, libdir = "lib")
    }
  }
