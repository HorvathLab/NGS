...
for snvchr, snvpos, ref, alt, snvextra in snvdata:
    ...
    phased = 0
    for si, alf in enumerate(opt.alignments):
        phased += sum(map(lambda t: counts1[(t[0], t[1], si)], [
    ...
    for si, alf in enumerate(opt.alignments):
        ...
        nsnvph0 = sum(map(lambda nuc: counts1[(nuc, 0, si)], map(str.strip,alt.split(','))))
        nsnvph1 = sum(map(lambda nuc: counts1[(nuc, 1, si)], map(str.strip,alt.split(','))))
        nsnvph2 = sum(map(lambda nuc: counts1[(nuc, 2, si)], map(str.strip,alt.split(','))))
        nsnvphased = nsnvph1 + nsnvph2
        nphased = nrefphased + nsnvphased
        # [ os.path.split(alf)[1].rsplit('.', 1)[0] ] + \
        row = [ snvchr, snvpos, ref, alt ] + \
	      map(snvextra.get,extrasnvheaders) + \
