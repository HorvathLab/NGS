...
#============================================================================================================== 
headers = ['#CHROM', 'POS',  'ID'  ,'REF',   'ALT',  'QUAL' ,  'FILTER' , 'INFO', 'FORMAT','MCF7']
def all_filteration(d):
			  
# Opening, Reading the Exonic coordinates from ensemble database, and opening the output file to write to
   ...
   # file = f_list[1]
    ...
    with open(outfile,'w') as outpt:
	 ...
	 outpt.write("\t".join(headers) + '\n')
         variant_count = 0 # Initiating a counter to looping around the list of variants (vcf file) 
	 ...
	 for row in reader:   # This loop belongs to the exonic coordinates file
	    ...
	    if (variant_count <len(d)):    # This is to check that counter doesn't outrun the end of the vcf list. 
...
#		             if (int(split_DP4_Colmn[2]) >=1 and int(split_DP4_Colmn[3]) >=1):
			      ...
			      if (exonic_start_pos<=variant_pos<=exonic_end_pos):
				    ...
				    #print "Check in One:", exonic_start_pos, " ", variant_pos, " ", exonic_end_pos
			      ...
			      #print "NOT IN:", exonic_start_pos, " ", variant_pos, " ", exonic_end_pos		
				   # print " Done FILTER"
		  	   # When we are done reading the variants list , then exit
			   ...
			   if 'MT' not in d[variant_count][0]:
                ...
                while (variant_pos > exonic_end_pos and chrom_variant < chrom_exonic_coord):
			     ...
			     if (variant_count == len(d)-1):
                                break
		             variant_count += 1
			     #print  d[variant_count][0], d[variant_count][1], variant_count
				...
				if d[variant_count][0] != 'Y':
                                   ...
                                   chrom_variant = d[variant_count][0]
                             variant_pos = int(d[variant_count][1])
			    # print"hi ya rab", chrom_variant, " ", variant_pos, "and", chrom_exonic_coord," ", exonic_end_pos 
			     ...
			     if (chrom_exonic_coord == chrom_variant):
			       ...
			       #     print "hi"
                                    # writing to the file the passed (filtered) variants
				    #print "Check in:", exonic_start_pos, " ", variant_pos, " ", exonic_end_pos		
                                    outpt.writelines("\t".join(d[variant_count-1]) + '\n')
