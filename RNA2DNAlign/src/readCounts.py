...
for snpchr,snppos,ref,alt,snpextra in snpdata:
    ...
    reads=[]; total = Counter(); badread = Counter()
    snppos1 = snppos - 1
	...
	counted = sum(map(lambda t: counts[(t[0],t[1],si)], [(n,d) for n in 'ACGT' for d in 'FR']))
	...
	counted = sum(map(lambda t: counts[(t[0],t[1],si)], [(n,d) for n in 'ACGT' for d in 'FR']))
        ...
        row = [ snpchr,snppos,ref,alt ] + \
              ...
              [ nsnpf, nsnpr,
                ...
                nsnp, nref, 
                counted,
                100.0*(total[si]-badread[si,'Good'])/float(total[si]) if total[si] != 0 else 0.0,
                -1,-1,-1,-1,-1,
                notherf,notherr,
                nother,
...
for i in range(len(outrows)):
    ...
    homovarsc = max(0.0,min(pvscore(r["NotHetFDR"]),pvscore(r["NotHomoRefFDR"]))-pvscore(r["NotHomoVarFDR"]))
    homorefsc = max(0.0,min(pvscore(r["NotHetFDR"]),pvscore(r["NotHomoVarFDR"]))-pvscore(r["NotHomoRefFDR"]))
    hetsc = max(0.0,min(pvscore(r["NotHomoRefFDR"]),pvscore(r["NotHomoVarFDR"]))-pvscore(r["NotHetFDR"]))
